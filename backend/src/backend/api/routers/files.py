...
async def get_record_inference_result(
    ...
    target_format: SupportedFormat = SupportedFormat.RAW,
    ...
    # Determine the original file extension from the mime type.
    file_extension = (
        mimetypes.guess_extension(record.mime_type).lower()
        if target_format == SupportedFormat.RAW
        else target_format.value
    )
    ...
    # Use the snippet service to load the file, extract the snippet, and convert it.
    try:
        snippet_buffer = generate_snippet_buffer(
            file_path, target_format, start_ms, end_ms, padding_ms
        )
    ...
    # Obtain the content from the snippet buffer.
    ...
    return StreamingResponse(
        io.BytesIO(content), media_type=record.mime_type, headers=full_headers
    )

...
class Jobs(Base):
    ...
    status = mapped_column(
        ...
        nullable=False,
        server_default=text("'running'::job_status"),
    )
...
class Labels(Base):
    ...
    model_labels: Mapped[List["ModelLabels"]] = relationship(
        "ModelLabels", uselist=True, back_populates="label"
    )
    events: Mapped[List["Events"]] = relationship(
        "Events", uselist=True, back_populates="label"
    )
    model_inference_results: Mapped[List["ModelInferenceResults"]] = relationship(
        "ModelInferenceResults", uselist=True, back_populates="label"
    )
...
class Locations(Base):
    ...
    sites: Mapped[List["Sites"]] = relationship(
        "Sites", uselist=True, back_populates="location"
    )
...
class Models(Base):
    ...
    events: Mapped[List["Events"]] = relationship(
        "Events", uselist=True, back_populates="model"
    )
    model_inference_logs: Mapped[List["ModelInferenceLogs"]] = relationship(
        "ModelInferenceLogs", uselist=True, back_populates="model"
    )
    model_inference_results: Mapped[List["ModelInferenceResults"]] = relationship(
        "ModelInferenceResults", uselist=True, back_populates="model"
    )
...
class SetInformations(Base):
    ...
    record_regime_recording_duration = mapped_column(
        Numeric(9, 4), nullable=False, server_default=text("0")
    )
    record_regime_pause_duration = mapped_column(
        Numeric(9, 4), nullable=False, server_default=text("0")
    )
    record_count = mapped_column(BigInteger, nullable=False, server_default=text("0"))
    record_duration = mapped_column(
        Numeric(14, 4), nullable=False, server_default=text("0")
    )
    corrupted_record_count = mapped_column(
    ...
    sets: Mapped[List["Sets"]] = relationship(
        "Sets", uselist=True, back_populates="set_information"
    )
...
class ModelLabels(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="model_labels_label_id_fkey",
        ),
        ForeignKeyConstraint(
            ["model_id"],
            ["models.id"],
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="model_labels_model_id_fkey",
        ),
...
class Sets(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="sets_set_informations_fkey",
        ),
    ...
    set_information: Mapped[Optional["SetInformations"]] = relationship(
        "SetInformations", back_populates="sets"
    )
    sets_sites_selections: Mapped[List["SetsSitesSelections"]] = relationship(
        "SetsSitesSelections", uselist=True, back_populates="set"
    )
...
class Sites(Base):
    ...
    prefix = mapped_column(Text, nullable=False)
    updated_at = mapped_column(DateTime)

    location: Mapped["Locations"] = relationship("Locations", back_populates="sites")
    records: Mapped[List["Records"]] = relationship(
        "Records", uselist=True, back_populates="site"
    )
    sets_sites_selections: Mapped[List["SetsSitesSelections"]] = relationship(
        "SetsSitesSelections", uselist=True, back_populates="site"
    )
    site_directories: Mapped[List["SiteDirectories"]] = relationship(
        "SiteDirectories", uselist=True, back_populates="site"
    )
    site_reports: Mapped[List["SiteReports"]] = relationship(
        "SiteReports", uselist=True, back_populates="site"
    )
...
class Records(Base):
    ...
    events: Mapped[List["Events"]] = relationship(
        "Events", uselist=True, back_populates="record"
    )
    model_inference_logs: Mapped[List["ModelInferenceLogs"]] = relationship(
        "ModelInferenceLogs", uselist=True, back_populates="record"
    )
    model_inference_results: Mapped[List["ModelInferenceResults"]] = relationship(
        "ModelInferenceResults", uselist=True, back_populates="record"
    )
...
class SetsSitesSelections(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="RESTRICT",
            name="sets_sites_selections_set_id_fkey",
        ),
        ForeignKeyConstraint(
            ["site_id"],
            ["sites.id"],
            ondelete="CASCADE",
            onupdate="RESTRICT",
            name="sets_sites_selections_site_id_fkey",
        ),
    ...
    site: Mapped["Sites"] = relationship(
        "Sites", back_populates="sets_sites_selections"
    )
...
class SiteDirectories(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="site_directories_site_id_fkey",
        ),
...
class ModelInferenceLogs(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="model_inference_logs_model_id_fkey",
        ),
        ForeignKeyConstraint(
            ["record_id"],
            ["records.id"],
            ondelete="CASCADE",
            onupdate="CASCADE",
            name="model_inference_logs_record_id_fkey",
        ),
        PrimaryKeyConstraint("id", name="model_inference_logs_pkey"),
        UniqueConstraint(
            "model_id", "record_id", name="model_inference_log_model_id_record_id_key"
        ),
        {
    ...
    model: Mapped["Models"] = relationship(
        "Models", back_populates="model_inference_logs"
    )
    record: Mapped["Records"] = relationship(
        "Records", back_populates="model_inference_logs"
    )
...
class ModelInferenceResults(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["model_id"], ["models.id"], ondelete="CASCADE", name="fk_model"
        ),
        ForeignKeyConstraint(
            ["record_id"], ["records.id"], ondelete="CASCADE", name="fk_record"
        ),
    ...
    label: Mapped["Labels"] = relationship(
        "Labels", back_populates="model_inference_results"
    )
    model: Mapped["Models"] = relationship(
        "Models", back_populates="model_inference_results"
    )
    record: Mapped["Records"] = relationship(
        "Records", back_populates="model_inference_results"
    )
